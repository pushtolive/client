...
class Client
{
    ...
    protected function zipPackPath(string $path): string
    {
        ...
        // Create the zippack
        $zipPackFileName = sprintf('/tmp/ptlz_%s', date('Ymd_His'));
        $zipPackCompressedFileName = $zipPackFileName.'.xz';
        mkdir($zipPackFileName);
        $zipPack = new LocalFilesystemAdapter($zipPackFileName);
        ...
        shell_exec("cd {$zipPackFileName}; tar cfv {$zipPackCompressedFileName} --use-compress-program='xz -9v' .");

        return $zipPackCompressedFileName;
    }
